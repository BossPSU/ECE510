// gelu_direct_lut.sv — Synthesizable direct GELU LUT (M4 update)
//
// 256-entry x 32-bit Q16.16 ROM holding GELU(x) for x in [-4, +4).
// Two registered read ports (addr_lo, addr_hi) emit data on the next
// clock edge — used by gelu_unit_lut to fetch adjacent entries for
// linear interpolation in a single LUT cycle.
//
// Contents are produced by gen_lut_mem.py and live in
// gelu_lut_direct.mem (half-open grid: entry[i] = GELU(-4 + i/32)).
